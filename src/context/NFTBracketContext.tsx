...
import type { Matchup, NFTMetadata, NFTPick, Region, Team, UserMatchup } from "~/types/bracket";
...
const convertNFTPickToUserMatchup = (
  ...
  if (!originalMatchup) {
    console.error(`Matchup with ID ${nftPick.id} not found in matchups:`, matchups.map(m => m.id));
    throw new Error(`Matchup with ID ${nftPick.id} not found`);
  }
  
  console.log(`Converting pick for matchup ${nftPick.id}, top team: ${nftPick.topTeam.id}, bottom team: ${nftPick.bottomTeam.id}, region: ${originalMatchup.region}`);
  
  // Find the teams by ID - handle both string and number IDs
  const topTeam = teams.find(t => t.id === nftPick.topTeam.id || t.id === String(nftPick.topTeam.id)) ?? null;
  const bottomTeam = teams.find(t => t.id === nftPick.bottomTeam.id || t.id === String(nftPick.bottomTeam.id)) ?? null;
...
export const NFTBracketProvider: React.FC<NFTBracketProviderProps> = ({
  ...
  const [currentRound, setCurrentRound] = useState<Matchup["round"]>("Round of 64");
  ...
  const regionPairs = useMemo<[Region[], Region[]]>(() => {
    ...
    const result: [Region[], Region[]] = pairs.length >= 2
      ? [pairs[0] ?? [], pairs[1] ?? []]
      : [pairs[0] ?? [], []];
  ...
  useEffect(() => {
    if (isClient && matchups?.length && teams?.length && metadata.data.picks) {
      ...
      try {
        ...
        const nftUserPicks = metadata.data.picks.map(nftPick => {
          try {
            const userMatchup = convertNFTPickToUserMatchup(nftPick, teams, matchups);
            return userMatchup;
          } catch (error) {
            console.error("Error converting NFT pick to UserMatchup:", error, nftPick);
            return null;
          }
