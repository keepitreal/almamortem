...
function calculateTeamWins(bracket: UserMatchup[]): Map<number, number> {
  ...
  for (const matchup of sortedMatchups) {
    ...
    if (matchup.winner) {
      ...
      if (!matchup.bottomTeam?.id.includes(teamId.toString()) && !matchup.topTeam?.id.includes(teamId.toString())) {
        throw new Error(`Team ${teamId} won matchup ${matchup.id} but wasn't a participant`);
      }
  ...
  for (const [teamId, wins] of winCounts) {
    if (wins > 6) {
      throw new Error(`Team ${teamId} has ${wins} wins, which is impossible in a March Madness bracket`);
    }
  }

  // Validate that exactly one team has 6 wins (the champion)
  const champions = Array.from(winCounts.entries()).filter(([, wins]) => wins === 6);
  if (champions.length > 1) {
    throw new Error('Multiple teams have 6 wins - only one team can be champion');
  }
  if (sortedMatchups.length === 63 && champions.length === 0) {
    throw new Error('No team has 6 wins - must have one champion in a complete bracket');
  }
