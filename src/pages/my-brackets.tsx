        ...
        {!isLoading && !error && ownedTokenIds.length > 0 && (
          <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            {ownedTokenIds.map((tokenId) => (
              <div key={tokenId} className="card overflow-hidden border shadow-md">
                <div className="p-4">
                  <h2 className="mb-2 text-xl font-semibold">Bracket #{tokenId}</h2>
                  <Link 
                    href={`/bracket/${tournamentId}/view/${tokenId}`}
