...
contract GameScoreOracle is ConfirmedOwner, FunctionsClient {
    ...
    string public constant SOURCE =
        ...
        "    if (!competition.notes || !competition.notes.some(note => note.headline.toLowerCase().includes(\"men's basketball championship\") && !note.headline.toLowerCase().includes(\"first four\"))) return;"
